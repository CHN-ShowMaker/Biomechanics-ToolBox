# -*- coding: utf-8 -*-
"""
原地纵跳分析脚本 (适配4.0核心模块，支持多力板，长格式输出)
功能：读取C3D，获取所有力板数据，遍历每块力板进行分析，
      检测腾空、起跳峰、落地冲击，计算腾空时间，保存曲线。
...
def analyze_countermovement_jump(c3d_file_path, output_dir='.', export_opensim=False):
    print("\n" + "="*50)
    print(f"开始分析原地纵跳文件: {os.path.basename(c3d_file_path)}")
    print(f"Starting CMJ analysis: {os.path.basename(c3d_file_path)}")
    print("="*50)
    ...
    for plate_idx, plate_dict in enumerate(plates_data):
        ...
        img_filename = os.path.basename(c3d_file_path).replace('.c3d', f'_plate{plate_idx+1}_cmj.png')
        save_path = os.path.join(img_subdir, img_filename)
        plot_utils.plot_force_with_events(
            force=force_filt,
            fs=fs,
            events=events,
            title=f'原地纵跳分析 (力板 {plate_idx+1} - {side_display}) / CMJ (Plate {plate_idx+1} - {side_display})',
            save_path=save_path
        )
