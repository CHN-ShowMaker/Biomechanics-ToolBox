...
def main():
    ...
    for filename in sorted(c3d_files):
        ...
        try:
            ...
            print(f"  检测到力板数量: {num_plates} / Plates detected: {num_plates}")

            for plate_idx, plate_dict in enumerate(plates_data):
                print(f"  --- 力板 {plate_idx+1} / Plate {plate_idx+1} ---")
                # 获取该板的校准矩阵
                ...
                fz_calibrated = np.max(plate_dict['Fz'])
                ...
                if plate_idx < len(plates_config):
                    fz_label = plates_config[plate_idx].get('force_vz')
                    if fz_label:
                    ...
                    print("    无配置信息，跳过原始值对比 / No config info, skipping raw value comparison")
