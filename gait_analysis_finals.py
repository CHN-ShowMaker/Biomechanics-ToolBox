# -*- coding: utf-8 -*-
"""
步态分析脚本 (适配3.0核心模块，双语版)
功能：读取C3D，调用新模块获取校准后的垂直力数据，检测触地/离地事件，
      计算支撑时间、峰值力，绘图，可选导出OpenSim文件，并自动保存结果到Excel汇总表。
"""
...
def analyze_gait(c3d_file_path, output_dir='.', export_opensim=False):
    ...
    步态分析主函数
    参数:
        c3d_file_path: C3D文件路径
        output_dir: 输出目录（图片和OpenSim文件保存位置）
        export_opensim: 是否导出OpenSim文件
    返回:
        结果字典，包含提取的指标
    """
    # 1. 读取C3D并获取力通道（使用新函数）
    ...
    if len(hs) > 0 and len(to) > 0:
        ...
        print(f"支撑时间（前{n_steps}步）: {stance_times} / Stance times (first {n_steps} steps): {stance_times}")
        avg_stance = np.mean(stance_times)
        print(f"平均支撑时间: {avg_stance:.3f} s / Mean stance time: {avg_stance:.3f} s")
    else:
        avg_stance = None
        print("未检测到完整的触地/离地事件。 / No complete foot strike/toe-off events detected.")
