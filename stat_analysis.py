...
def get_input(prompt, default=None):
    """获取用户输入，可带默认值（英文提示在下一行）"""
    if default:
        value = input(f"{prompt} (默认 {default}): ").strip()
        print(f"{prompt} (default {default}): {value}")
        if value == '':
            value = default
    else:
...
def main():
    ...
    # 获取分组列名
    group_col = get_input("请输入分组列名（用于比较的组别）/ Enter the column name for the grouping variable")
    if group_col not in df.columns:
        print(f"错误：列 '{group_col}' 不存在。/ Error: Column '{group_col}' does not exist.")
        return
    ...
    if len(groups) == 2:
        ...
        print(f"\n组 {groups[0]} 描述统计：/ Group {groups[0]} descriptive statistics:")
        print(group1.describe())
        print(f"\n组 {groups[1]} 描述统计：/ Group {groups[1]} descriptive statistics:")
        ...
        if perform_test:
            ...
            print("样本量不足，跳过统计检验。/ Insufficient sample size, skipping statistical tests.")
        ...
        print("\n各组的描述统计：/ Descriptive statistics per group:")
        ...
        print("分组数不足，无法进行组间比较。/ Insufficient number of groups for comparison.")
    ...
    print(f"描述统计已保存至 descriptive_stats.xlsx / Descriptive statistics saved to descriptive_stats.xlsx")
