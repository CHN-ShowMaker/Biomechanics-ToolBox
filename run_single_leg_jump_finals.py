...
def analyze_single_leg_jump(c3d_file_path, output_dir='.', export_opensim=False):
    ...
    peaks_before, _ = find_peaks(segment_before, height=np.max(segment_before)*0.3, distance=fs*0.1)
    ...
    events = {
        '离地': takeoff_frame,
        '落地': landing_frame,
        '起跳蹬伸峰': takeoff_peak_frame,
        '落地冲击峰': landing_peak_frame,
    }
    save_path = os.path.join(output_dir, os.path.basename(c3d_file_path).replace('.c3d', '_single_jump.png'))
    plot_utils.plot_force_with_events(force_filt, fs, events, '跑动单腿跳分析', save_path)
    ...
    result = {
        ...
        '动作类型': '跑动单腿跳',
        '起跳蹬伸峰_N': takeoff_peak_force,
        '腾空时间_s': flight_time,
        '落地冲击峰_N': landing_peak_force,
        '离地帧': takeoff_frame,
        '落地帧': landing_frame,
    }
    excel_path = os.path.join(raw_folder, '单腿跳累计版.xlsx')
