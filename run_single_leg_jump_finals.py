# -*- coding: utf-8 -*-
"""
跑动单腿跳分析脚本 (适配4.0核心模块，支持多力板，长格式输出)
功能：读取C3D，获取所有力板数据，遍历每块力板进行分析，
      检测腾空、起跳蹬伸峰、落地冲击，计算腾空时间，保存曲线。
...
def analyze_single_leg_jump(c3d_file_path, output_dir='.', export_opensim=False):
    print("\n" + "="*50)
    print(f"开始分析跑动单腿跳文件: {os.path.basename(c3d_file_path)}")
    print(f"Starting single-leg jump analysis: {os.path.basename(c3d_file_path)}")
    print("="*50)
    ...
    for plate_idx, plate_dict in enumerate(plates_data):
        ...
        img_filename = os.path.basename(c3d_file_path).replace('.c3d', f'_plate{plate_idx+1}_single_jump.png')
        save_path = os.path.join(img_subdir, img_filename)
        plot_utils.plot_force_with_events(
            force=force_filt,
            fs=fs,
            events=events,
            title=f'单腿跳分析 (力板 {plate_idx+1} - {side_display}) / Single‑Leg Jump (Plate {plate_idx+1} - {side_display})',
            save_path=save_path
        )
    ...
    # OpenSim 导出（仍只导出第一块力板？可改为所有板，但这里保持简单）
