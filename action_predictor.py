# -*- coding: utf-8 -*-
"""
动作特征提取器 (action_features.py)
功能：读取C3D文件，提取垂直力信号的关键特征（腾空时间、峰数量、最小力、腾空次数等），
...
def extract_features(c3d_file, force_threshold=FORCE_THRESHOLD,
    ...
    返回字典，包含以下字段：
        ...
        - 'flight_count': 腾空次数（力低于阈值的连续区域个数）
...
def process_single_file(file_path, plot=False):
    ...
    if features:
        print(f"\n{os.path.basename(file_path)} 特征:")
        print(f"  最小力: {features['min_force']:.1f} N")
        print(f"  最大力: {features['max_force']:.1f} N")
        print(f"  峰数量: {features['num_peaks']}")
        print(f"  最长腾空时间: {features['max_flight_duration']:.3f} s")
        print(f"  腾空次数: {features['flight_count']}")
        print(f"  是否存在腾空: {'是' if features['flight_exists'] else '否'}")
    ...
    for f in c3d_files:
        ...
        if features:
            results.append({
                ...
                '最小力_N': features['min_force'],
                '最大力_N': features['max_force'],
                '峰数量': features['num_peaks'],
                '最长腾空时间_s': features['max_flight_duration'],
                '腾空次数': features['flight_count'],
                '是否存在腾空': features['flight_exists']
            })

    if results:
        print("\n特征汇总：")
        for res in results:
            print(f"{res['文件名']}: 最小力={res['最小力_N']:.1f}, 峰数={res['峰数量']}, "
                  f"腾空时间={res['最长腾空时间_s']:.3f}s, 腾空次数={res['腾空次数']}")

        if HAS_PANDAS:
            df = pd.DataFrame(results)
            output_path = os.path.join(folder_path, '动作特征汇总.xlsx')
            df.to_excel(output_path, index=False)
            print(f"\n特征汇总表已保存至：{output_path}")
        else:
            print("\n（未安装 pandas，未生成Excel文件）")
    else:
        print("没有成功提取任何文件的特征。")

def interactive_loop():
    print("动作特征提取器（批量处理）")
    print("输入文件或文件夹路径，输入 'q' 退出。")
    while True:
        path = input("\n请输入路径: ").strip()
