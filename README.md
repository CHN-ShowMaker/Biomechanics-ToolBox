...
Manual configuration (manual_config.py): run the script, enter folder path interactively; manually select the vertical force channel for each file; other components (Fx, Fy, Mx, My, Mz, COPx, COPy) are automatically matched.
...
Configure channels (choose according to data regularity):

Manual configuration (interactive per file):

bash
python manual_config.py
Then follow the prompts to enter the folder path and select the vertical force channel index for each file.
